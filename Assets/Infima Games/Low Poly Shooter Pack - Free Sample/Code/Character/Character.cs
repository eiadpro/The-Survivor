...
namespace InfimaGames.LowPolyShooterPack
{
	...
	public sealed class Character : CharacterBehaviour
	{
		...
		{
			init.GetComponent<TargetScript>().initial();
		...
		public void OnTryInventoryNext(InputAction.CallbackContext context)
		{
			...
			switch (context)
			{
				...
				case {phase: InputActionPhase.Performed}:
					...
					float scrollValue = context.valueType.IsEquivalentTo(typeof(Vector2)) ? Mathf.Sign(context.ReadValue<Vector2>().y) : 0f;
					...
					if (CanChangeWeapon() && (indexCurrent != indexNext)&&m.havegun1)
